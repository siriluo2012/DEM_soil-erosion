...
def visualize(real_data, predict_data, predicted_data, idx):
    ...
    plt.show()
    plt.savefig(MP['dir']+'/mask_comparison_idx%d.png'%(idx))
...
def train_and_test_model(Data, MP):
    ...
    print("binary XE score = %f" % model.evaluate(X_true, Y_true))
    print(Y_preds)
    print("###################################")
    print("###################################")
...
if __name__ == '__main__':
    ...
    MP['save_models'] = 1
    MP['save_dir'] = MP['dir'] + '/model/model.h5'
    ...
    input_train, input_test, label_train, label_test = train_test_split(
            input_data, label_data, test_size=0.1, random_state=21)

    input_train, input_dev, label_train, label_dev = train_test_split(
            input_train, label_train, test_size=0.1, random_state=21)	
